...
impl QuicStream {
    ...
    pub async fn send(&mut self, mut message: Message) -> Result<(), NetError> {
        ...
        let bytes = Bytes::from(message.to_vec().map_err(|e| NetError::from(e))?);
    ...
    pub async fn send_bytes(&mut self, bytes: Bytes) -> Result<(), NetError> {
        ...
        let bytes_len = u16::try_from(bytes.len())
            .map_err(|_e| NetError::from(ProtoError::from(ProtoErrorKind::MaxBufferSizeExceeded(bytes.len()))))?;
    ...
    pub async fn receive(&mut self) -> Result<DnsResponse, NetError> {
        ...
        let message = Message::from_vec(&bytes).map_err(|e| NetError::from(e))?;
        ...
        if message.id() != 0 {
            ...
            return Err(NetError::from(NetErrorKind::QuicMessageIdNot0(message.id())));
        }

        DnsResponse::from_buffer(bytes.to_vec()).map_err(|e| NetError::from(e))
    }
    ...
    pub async fn receive_bytes(&mut self) -> Result<BytesMut, NetError> {
        ...
        let mut len = [0u8; 2];
        self.receive_stream.read_exact(&mut len).await.map_err(|e| NetError::from(e))?;
